...
//zye
//static NSString * const kDefaultRegularExpression  = @"(\\d+);(_(telstra|soprano|att|vodafone)+)";
static NSString * const kDefaultRegularExpression  = @"\\d+";
...
//zye
@property (weak) IBOutlet NSTextField *ignoreSimilarRegExpTextField;
...
//zye
@property (strong, nonatomic) NSArray *regExpArray;
...
- (IBAction)onSearchButtonClicked:(id)sender {
    ...
    //zye
    //save regular expressions before setUIEnabled:NO
    self.regExpArray = [self regularExps];
    ...
    if (self.resultsTableView.numberOfSelectedRows > 0) {
        ...
        while (index != NSNotFound) {
            if (index < results.count) {
                ...
                [[NSFileManager defaultManager] removeItemAtURL:[NSURL fileURLWithPath:info.fullPath] error:nil];
            }
...
- (void)tableViewDoubleClicked {
    ...
    [[NSWorkspace sharedWorkspace] selectFile:info.fullPath inFileViewerRootedAtPath:@""];
}
...
//zye
- (NSArray *)regularExps {
    return [self ignoreSimilarRegExps];
}
...
//zye
- (NSArray *)ignoreSimilarRegExps {
    NSString *regExps = self.ignoreSimilarRegExpTextField.stringValue;
    
    if (!regExps|| (regExps.length == 0))
        regExps = kDefaultRegularExpression;
    
    return [regExps componentsSeparatedByString:@";"];
}
...
- (void)setUIEnabled:(BOOL)state {
    ...
    //zye
    [_ignoreSimilarRegExpTextField setEnabled:state];
    ...
    if (self.isFileDone && self.isStringDone) {
        ...
        for (NSString *name in resNames) {
            if (![[ResourceStringSearcher sharedObject] containsResourceName:name]) {
                ...
                if (!self.regExpArray || !self.regExpArray.count) {
                    [self showAlertWithStyle:NSWarningAlertStyle title:@"Similar Regular Expression Error" subtitle:@"Regular Expression is invalid"];
                    return;
                } else {
                    regExps = self.regExpArray;
                }
                
                //zye
                //                if (!self.ignoreSimilarCheckbox.state
                //                    || ![[ResourceStringSearcher sharedObject] containsSimilarResourceName:name]) {
                if (!self.ignoreSimilarCheckbox.state || (![[ResourceStringSearcher sharedObject] containsSimilarResourceName:name regExps:regExps])) {
...
- (BOOL)usingResWithDiffrentDirName:(ResourceFileInfo *)resInfo
{
    ...
    NSDirectoryEnumerator *fileEnumerator = [[NSFileManager defaultManager] enumeratorAtPath:resInfo.fullPath];
