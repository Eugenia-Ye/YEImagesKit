...
- (NSImage *)image {
    if ([StringUtils isImageTypeWithName:self.name]) {
        return [[NSImage alloc] initByReferencingFile:self.fullPath];
    }
    
    if ([self.name hasSuffix:kSuffixImageSet]) {
        NSError *error = nil;
        NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:self.fullPath error:&error];
        if (files.count == 0) {
            return NO;
        }
        for (NSString *file in files) {
            if ([StringUtils isImageTypeWithName:file]) {
                return [[NSImage alloc] initByReferencingFile:[self.fullPath stringByAppendingPathComponent:file]];
            }
        }
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSString *path in resPaths) {
            ...
            if (!tempResNameInfoDict[keyName]) {
                ...
                info.name = name;
                info.fullPath = path;
                info.relativePath = [StringUtils getRelativePathFromFullPath:path withProjectPath:projectPath];
                info.fileSize = [YEFileUtils fileSizeAtPath:path isDir:&isDir];
                info.isDir = isDir;
